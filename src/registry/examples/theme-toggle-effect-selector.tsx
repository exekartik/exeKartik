...
export function ThemeToggleEffectSelector() {
  ...
  useLayoutEffect(() => {
    if (effectName && effectName in EFFECTS) {
      addEffectStyle(
        document,
        EFFECTS[effectName as EffectName]?.css || "",
        EFFECT_STYLE_ID
      )
    } else {
  ...
  return (
    <Select
      value={effectName}
      onValueChange={(selectedEffect) => setEffectName(selectedEffect)}
    >
      <SelectTrigger
        className="w-50 *:data-[slot=select-value]:gap-2"
        aria-label="Select Effect"
      >
        <SelectValue placeholder="Custom Effect (Optional)" />
      </SelectTrigger>
